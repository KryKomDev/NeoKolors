...
using NeoKolors.Tui.Fonts.V2.Serialization.Xml;
...
public static class NKFontSerializer {
    ...
    #if DEBUG || WIP
    public static string CONF_SCHEMA_LOCAL_PATH = "";
    public static string MAP_SCHEMA_LOCAL_PATH = "";
    #endif
    
    private static readonly XmlSchemaSet CONFIG_SCHEMA;
    private static readonly XmlSchemaSet MAP_SCHEMA;
    ...
    static NKFontSerializer() {
        ...
        #else
        CONFIG_SCHEMA = GetConfigSchemaLocal();
        MAP_SCHEMA = GetMapSchemaLocal();
        #endif
    }
    
    private static readonly Lazy<Task<(XmlSchemaSet ConfigSchema, XmlSchemaSet MapSchema)>> SCHEMA_INITIALIZATION 
    ...
    private static async Task<XmlSchemaSet> GetConfigSchemaAsync() {
        var schemas = await SCHEMA_INITIALIZATION.Value;
    ...
    private static XmlSchemaSet GetConfigSchemaLocal() {
        LOGGER.Info("Reading font config XSD...");
        
        string raw = File.ReadAllText(CONF_SCHEMA_LOCAL_PATH);
    ...
    private static XmlSchemaSet GetMapSchemaLocal() {
        LOGGER.Info("Reading font map XSD...");
        
        string raw = File.ReadAllText(MAP_SCHEMA_LOCAL_PATH);
        ...
        return new NKFont(
            new NKFontInfo(
                (Equals(config.Name, null)
                    ...
                    : config.Name)!,
                config.Monospace 
                    ? MonospaceInfo.Mono(config.LetterWidth.Value, config.LetterHeight.Value) 
                    : MonospaceInfo.Var(),
                config.Ligatures,
                config.Kerning,
                config.CharSpacing,
