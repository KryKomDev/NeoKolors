...
internal static class Program {
    ...
    static IAsciiFont? LoadFont(string fontKey) {
        ...
        // 2. Try loading from binary file path
        if (File.Exists(fontKey)) {
            string fullPath = Path.GetFullPath(fontKey);

            // Read first few bytes to check if it's a binary font
            try {
                ...
                // Ignore and try general loaders
            }
        }

        // 3. Try loading as general XML path (directory or zip)
