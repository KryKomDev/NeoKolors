...
public class ToolTip : ContentControl {
    ...
    private Point2D _offset = new(0, 1);
    ...
    public Point2D Offset {
        get => _offset;
        set {
            if (_offset.X == value.X && _offset.Y == value.Y)
                return;
    ...
    protected override Size2D ArrangeOverride(Size2D finalSize) {
        if (Content is IElement element) {
            ...
            element.Arrange(new Area2D(contentPos, RenderLayout.Content.Size));
        }
    ...
    protected override void RenderCore(ICharCanvas canvas) {
        ...
        if (Content is IElement element) {
            ...
            var text = Content.ToString() ?? string.Empty;
            canvas.Place(text, pos + RenderLayout.Content.Lower, RenderLayout.Content.SizeX, HorizontalAlign.LEFT);
        }
    }
